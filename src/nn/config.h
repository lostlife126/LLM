...
struct ModelConfig {
  ...
  // Нормировка запросов и ключей перед их скалярным произведением.
  //
  // Оценки внимания — это произведение двух векторов, длина которых ничем не
  // ограничена. По ходу обучения они растут, softmax насыщается, градиент
  // через него исчезает, и слой перестаёт учиться. Деление на масштаб
  // ограничивает длину и снимает эту проблему в корне: скалярное произведение
  // двух нормированных векторов не может выйти за пределы размерности головы.
  //
  // Приём дешёвый — два вектора весов на слой — и в современных моделях
  // считается обязательным.
  bool qk_norm = false;

  // Коэффициент вспомогательных потерь, удерживающих логиты от дрейфа.
  //
  // Softmax не меняется при добавлении константы ко всем логитам, поэтому
  // ничто не мешает им уехать далеко от нуля целой группой: на потери это не
  // влияет, а точность float падает и экспонента подходит к переполнению.
  // Штраф на квадрат логарифма суммы экспонент возвращает их к нулю, почти не
  // меняя сами предсказания. Ноль выключает.
  float z_loss_coef = 0.0f;

  // Вероятность дропаута на путях остатка: выход внимания и выход FFN
  // зануляются поканально перед тем, как прибавиться к остатку.
  //
  // Ноль выключает, и это значение по умолчанию — ровно как в Llama. Приём
  // стоит отдельного слова: в больших моделях от него отказались, и не потому,
  // что он плох, а потому, что бороться стало не с чем. Переобучение возникает,
  // когда параметров много, а данных мало; у frontier-моделей наоборот, и
  // каждый пример виден один раз. У нас соотношение обратное, и это видно по
  // числам — потому развилка и заведена.
  //
  // Дропаута на весах внимания здесь нет намеренно: он лечит другое и добавил
  // бы вторую ручку, из-за которой замер отвечал бы сразу на два вопроса.
  float dropout = 0.0f;
  ...
  // Веса нормировок запросов и ключей: по одному вектору длины head_dim на
  // каждую, если QK-норма включена.
  int64_t qk_norm_parameter_count() const {
    return qk_norm ? 2 * head_dim() : 0;
  }
...
// Сторож для сравнения ниже.
//
// operator== перечисляет поля вручную, и забытое поле не даст ни ошибки, ни
// предупреждения — оно просто перестанет участвовать в сравнении. Последствие
// ровно то, ради предотвращения которого сравнение и написано: два описания,
// различающиеся только этим полем, окажутся равными, load_checkpoint примет
// чужой снимок, формы и имена весов сойдутся, и модель будет считать не то,
// что записано в файле.
//
// Размер структуры меняется при добавлении почти любого поля, поэтому
// несовпадение здесь остановит сборку и заставит автора заглянуть в
// operator==. Честно про предел: поле, помещающееся в уже существующий зазор
// выравнивания, размера не изменит, и такое добавление сторож пропустит.
// Полной защиты в C++14 без рефлексии не выходит, а эта ловит подавляющее
// большинство случаев и стоит одну строку.
//
// Число получено сборкой, а не подсчётом на бумаге: 7 int64, 5 float, 3
// перечисления и 3 bool с выравниванием на восемь.
static_assert(sizeof(ModelConfig) == 96,
              "размер ModelConfig изменился — проверьте, что новое поле "
              "добавлено и в operator==, и в write_config/read_config_body");
