...
void ModelConfig::validate() const {
  ...
  // как раз дробные.
  LLM_CHECK_MSG(norm_eps > 0.0f, "eps нормировки "
                                     << norm_eps
                                     << " должен быть положительным: при нуле "
                                        "нулевая строка даёт бесконечность");
  LLM_CHECK_MSG(rope_theta > 0.0f, "основание поворота "
                                       << rope_theta
                                       << " должно быть положительным");
  // Ноль допустим: это вырожденная, но осмысленная инициализация нулями.
  LLM_CHECK_MSG(init_std >= 0.0f,
                "разброс инициализации " << init_std << " отрицателен");
  LLM_CHECK_MSG(z_loss_coef >= 0.0f, "коэффициент z-loss "
                                         << z_loss_coef
                                         << " отрицателен: штраф стал бы "
                                            "поощрением");

  // Конечность проверяется отдельно, потому что сравнения выше её не
  // проверяют. NaN они ловят — с ним ложно любое сравнение, — а вот плюс
  // бесконечность проходит их все: она и больше нуля, и не меньше нуля.
  //
  // Прийти она может. Разбор JSON теперь отвергает литералы, не помещающиеся
  // в double, и разбор аргументов отвергает «inf» — но чекпоинт читает числа
  // сырыми байтами, и достаточно испорченного файла. Дальше это не падает:
  // при бесконечном rope_theta все частоты, кроме нулевой пары, обращаются в
  // нуль, то есть поворот позиций тихо выключается; при бесконечном norm_eps
  // нормировка делит на бесконечность и отдаёт нули. Ни то, ни другое не
  // выдаёт себя ничем, кроме результата.
  LLM_CHECK_MSG(std::isfinite(norm_eps),
                "eps нормировки не конечен: " << norm_eps);
  LLM_CHECK_MSG(std::isfinite(rope_theta),
                "основание поворота не конечно: " << rope_theta);
  LLM_CHECK_MSG(std::isfinite(init_std),
                "разброс инициализации не конечен: " << init_std);
  LLM_CHECK_MSG(std::isfinite(z_loss_coef),
                "коэффициент z-loss не конечен: " << z_loss_coef);
  // Дропаут ограничен сверху единицей, то есть бесконечность отсечена
  // проверкой выше. Названа здесь для полноты списка вещественных полей.
}
