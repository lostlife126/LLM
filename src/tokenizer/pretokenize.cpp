...
bool is_space_code(uint32_t code) {
  // \s в этих выражениях — свойство Unicode White_Space целиком, как его
  // понимает библиотека регулярных выражений эталонного токенизатора. Список
  // этот короткий и закрытый, поэтому выписан весь, а не «сколько нужно на
  // практике»: пропущенная точка не падает, а сдвигает границу куска, то есть
  // меняет номера токенов на тексте, где она встретилась.
  //
  // Двадцать пять точек: 0009..000D, 0020, 0085, 00A0, 1680, 2000..200A,
  // 2028, 2029, 202F, 205F, 3000. Не входят сюда 001C..001F, которые
  // str.isspace в Python пробелами считает, а Unicode — нет, и U+200B, у
  // которого пробел только в названии.
  return (code >= 0x09u && code <= 0x0Du) || code == 0x20u || code == 0x85u ||
