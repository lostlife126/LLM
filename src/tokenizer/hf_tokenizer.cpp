...
void HfTokenizer::apply_merges(std::vector<std::string>* symbols) const {
  ...
  while (symbols->size() > 1) {
    ...
    //
    // У этого рассуждения есть посылка, и её стоит назвать вслух: список
    // слияний получен обучением BPE, то есть склейка стоит в нём ПОЗЖЕ обеих
    // своих половин. Список, собранный руками и это нарушающий, — скажем, с
    // парой (ab, c) рангом ноль при (a, b) рангом пять — сделал бы минимум
    // убывающим, и тогда замена по одному пошла бы другим путём. Эталонная
    // реализация держится на том же свойстве: она разбирает очередь по
    // наименьшему рангу и тоже полагается на его неубывание. То есть с
    // настоящим файлом расхождения нет, а с подделанным его нет и у эталона.
    //
