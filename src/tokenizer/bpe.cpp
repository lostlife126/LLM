...
Bpe Bpe::load(const std::string& path) {
  ...
  // Отдельная проверка на нечитаемое число. Без неё нечисловое count
  // оставляло в переменной ноль — таково поведение operator>> при отказе, — и
  // словарь с настоящими слияниями загружался как пустой, молча. Это хуже
  // отказа: дальше такой словарь проходит и сверку размера с моделью (259
  // токенов не больше её словаря), и модель отвечает связно и мимо.
  LLM_CHECK_MSG(!file.fail(), "не прочитано число слияний в " << path);

  Bpe bpe;
  // Резерв — подсказка аллокатору, и брать её из файла как есть нельзя:
  // заявленные в заголовке 10^17 слияний обрывали загрузку не сообщением, а
  // std::bad_alloc из глубины vector. Предел ниже ничего не запрещает —
  // вектор дорастёт сам, если слияний действительно больше, — а миллион
  // слияний это уже 4 МБ текста, на фоне которых перевыделение незаметно.
  const std::size_t kReserveLimit = 1 << 20;
  bpe.merges_.reserve(count < kReserveLimit ? count : kReserveLimit);
