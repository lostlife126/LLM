...
class AdamW {
 public:
  ...
  // Сумма квадратов элементов с фиксированным порядком сложения. Открыта не
  // ради удобства: на неё опирается тест, который сверяет норму градиента с
  // прямолинейным подсчётом.
  static double sum_squares(const Tensor& tensor);
