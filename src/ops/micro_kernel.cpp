...
__attribute__((target("avx2,fma"))) void avx2_kernel(
  ...
  // Выравнивание — для единообразия с ядрами AVX-512 ниже, где оно
  // обязательно. Здесь команды 256-разрядные, а поддельный стек санитайзера
  // даёт как раз 32 байта, так что свёртка в выровненную была бы безопасна и
  // без этого.
  alignas(32) float tile[kAvx2M * kAvx2N];
...
__attribute__((target("avx512f,avx512bw,avx512vl"))) void avx512_kernel(
  ...
  // Выравнивание объявлено, а не оставлено компилятору.
  //
  // Плитка пишется и читается 512-разрядными командами по смещениям, кратным
  // 64. Записаны они невыровненными, но компилятор вправе свернуть их в
  // выровненные, если решит, что знает выравнивание локального массива.
  // Сегодня он так не делает — в выдаче стоит vmovups, а сам массив лежит по
  // смещению 48 от вершины стека, — но это его выбор, а не свойство кода.
  //
  // Стоит ему выбрать иначе, и сборка с санитайзером адресов падает: локальные
  // переменные уезжают на поддельный стек с выравниванием в 32 байта, и
  // 64-байтной команде достаётся адрес, кратный не тому числу. Это не
  // предположение — ровно так уже падало ядро экспоненты, см. комментарий у
  // хвостового буфера в ops/fast_exp.cpp. Явное выравнивание оставляет
  // переменную на настоящем стеке, и предположение компилятора становится
  // верным, каким бы он ни был.
  alignas(64) float tile[kAvx512M * kAvx512N];
...
//
// Замер на Raspberry Pi 5, чередованием, три раунда: 29.5 -> 30.2, 29.3 ->
// 30.4, 29.5 -> 30.4 GFLOPS на квадрате 512 в один поток. Три процента, и
// сходится с долей скалярной упаковки в профиле. Одиночными запусками этот
// эффект не ловится: первая попытка дала 29.6 против 30.1 и была прочитана
// как «ничего не дало».
...
void neon_kernel_rows_half(int64_t kc, const float* __restrict a,
  ...
  for (int64_t p = 0; p < kc; ++p) {
    ...
    // Загружается как шестнадцатиразрядные целые, а потом переистолковывается
    // вектор — а не читается сразу через указатель на __fp16.
    //
    // Разница не формальная. Half содержит uint16_t, и чтение его байт через
    // указатель на другой тип нарушает правила совместимости типов: сегодня
    // компилятор так и сделает, что написано, а на следующей версии вправе
    // предположить, что эти два указателя не смотрят на одну память.
    // Переистолкование ВЕКТОРА такого предположения не допускает, а стоит
    // ничего: vreinterpretq не порождает ни одной инструкции.
    const float16x8_t packed = vreinterpretq_f16_u16(
        vld1q_u16(reinterpret_cast<const uint16_t*>(b + p * bstride)));
...
const MicroKernelChoice* build_table(int* count) {
  ...
  if (!ready) {
    ...
    //
    // Ядро написано как `acc += a * b`, то есть раздельно, — и остаётся
    // раздельным на всех машинах. Раньше здесь стояло LLM_HAS_NEON != 0:
    // на aarch64 слитная команда входит в базовый набор, компилятор её
    // подставлял сам, и скалярное ядро оказывалось слитным. Признак говорил
    // про это правду, но правда была неудобная — арифметику выбирал
    // процессор, а не исходник, и то же самое происходило со всем остальным
    // кодом проекта, включая свёртки по строке, из которых собраны нормировки
    // и функция потерь. Обучение на Raspberry Pi и на обычной машине давало
    // разные модели; измерено.
    //
    // Теперь сборка идёт с -ffp-contract=off, склейки не происходит нигде, и
    // скалярное ядро раздельное везде — отсюда и false без условия. Договор,
