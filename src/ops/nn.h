...
// Слитая цепочка внимания: softmax(маска(scores * scale)).
//
// Собрана в одну операцию не ради красоты. Порознь это шесть проходов по
// тензору размера (матрицы x запросы x ключи): умножение на скаляр, маска и
// четыре прохода самого softmax. Слитая версия делает четыре прохода и только
// по видимой части строки, а её у каузальной маски в среднем половина; два
// промежуточных тензора при этом не выделяются вовсе.
//
// Результат совпадает с раздельной цепочкой побитово, и это свойство, а не
// совпадение: закрытые позиции дают после экспоненты ровно нуль, нуль не
// меняет ни максимум, ни сумму — а сумма считается по накопителям, номер
// которых у элемента зависит только от его индекса, так что укорочение строки
// не переставляет слагаемые.
Tensor masked_softmax(const Tensor& scores, float scale, int64_t query_offset);

// Обратный проход слитой цепочки. Маска в нём не участвует: на закрытых
// позициях выход нулевой, а производная softmax пропорциональна выходу,
// поэтому там и так получается нуль.
