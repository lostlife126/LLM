...
struct PredictionStats {
  ...
  // Среднее по строкам значение логарифма суммы экспонент — та самая
  // величина, квадрат которой штрафует z_loss. Softmax её не видит, поэтому
  // логиты могут уезжать от нуля целой группой, ничего не меняя в
  // предсказаниях. Здесь видно, уезжают ли: если log Z держится около нуля,
  // штрафовать нечего.
  float log_z = 0.0f;
};
