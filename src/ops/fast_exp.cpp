...
// появления этого файла, поэтому по скорости на таких машинах ничего не
// ухудшается.
//
// А по числам — ухудшается, и об этом сказано в заголовке: std::exp не
// определена стандартом до последнего разряда, поэтому на такой машине
// результат зависит ещё и от libc. Побитовая воспроизводимость держится там,
// где выбрано векторное ядро, — то есть на всякой aarch64 и всякой x86-64 с
// AVX2.
