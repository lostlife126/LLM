...
void write_safetensors(const std::string& path,
  ...
  // Заголовок дополняется пробелами так, чтобы данные начинались со
  // смещения, кратного восьми.
  //
  // Нашему читателю это безразлично: он берёт данные по смещению и копирует
  // их. Но файл, который мы называем safetensors, читают и другие, а читатель,
  // отображающий файл в память, обращается к массиву float прямо в отображении
  // — и на невыровненном начале спотыкается тем сильнее, чем шире у него
  // команда загрузки.
  //
  // Честно про источник: что именно делает эталонная реализация, здесь не
  // проверено — её исходник не читался. Довод другой и самодостаточный:
  // выравнивание ничего не стоит, ничего не ломает и снимает целый класс
  // чужих отказов. Пробелы законны внутри JSON, и то, что наш разбор их
  // переживает, проверено тестом рядом.
