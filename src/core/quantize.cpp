...
int scale_exponent(const Tensor& tensor, Precision precision) {
  ...
  // Бесконечность среди значений — отказ, а не «как-нибудь».
  //
  // NaN сюда не доходит: сравнение с ним ложно, и наибольшим он не станет.
  // А бесконечность станет, и дальше frexp получает бесконечное отношение.
  // Стандарт для такого случая значение показателя НЕ оговаривает; на этой
  // библиотеке выходит нуль, то есть масштаб молча не применяется вовсе, и
  // измеренная после этого погрешность относится не к формату, а к тому, что
  // масштаба не было. Ровно ту подмену, ради предотвращения которой написан
  // весь этот файл.
  LLM_CHECK_MSG(std::isfinite(largest),
                "в тензоре есть бесконечность: масштаб по нему не определён");
