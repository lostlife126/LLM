...
// Объявлено здесь, потому что нужно уже в Pool::run, а определено ниже: там
// оно стоит рядом с default_width, от которого зависит его начальное значение.
...
class Pool {
 public:
  ...
  void run(int tasks, const std::function<void(int)>& task) {
    ...
    // Будим ровно столько потоков, сколько нужно. Ограничений три, и каждое
    // своё: созданных потоков не больше, чем есть; вызывающий — тоже
    // участник, поэтому при двух задачах второй поток не нужен вовсе; и
    // ширина — сколько участников разрешено вообще.
    //
    // Ширина спрашивается здесь, а не только у вызывающих. Раньше она сюда не
    // доходила, и parallel_for её попросту не замечал: при ширине 1 и четырёх
    // ядрах шестнадцать задач всё равно расходились по четырём потокам —
    // измерено счётчиком различных идентификаторов потока. На счёт это не
    // влияло, потому что все боевые вызывающие сами считают число задач от
