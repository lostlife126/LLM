...
class Tensor {
 public:
  ...
  // Плотное размещение: элементы лежат подряд в порядке обхода формы. Только
  // для таких тензоров имеет смысл flat() и передача сырого указателя в ядра,
  // ожидающие непрерывную память.
  //
  // Это НЕ то же самое, что «шаги совпадают с contiguous_strides»: оси
  // размера 1 не ограничивают размещение, потому что индекс по ним всегда
  // ноль и их шаг никогда не участвует в вычислении адреса. Транспонирование
  // формы (1, 2, 3) даёт (2, 1, 3) с шагами (3, 6, 1) — плотное размещение с
  // шагами, отличными от (3, 3, 1).
