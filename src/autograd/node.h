...
//
// Значения остаются в float, но каждое проходит через представление
// половинной разрядности и возвращается: получается ровно та точность,
// которую дало бы настоящее хранение в fp16, при неизменном коде. Нужно, чтобы
// проверить сходимость обучения до того, как тип элемента тензора будет
// переделан — работа на дни, а риск не в ней, а в том, сойдётся ли вообще.
//
// Включается через переменную окружения LLM_FP16 или вызовом. По умолчанию
// выключено, и тогда проверка стоит одно сравнение на операцию.
//
// Затрагиваются два места, и оба обязательны: результаты операций (это
// активации прямого прохода) и вклады в градиент (обратный проход идёт мимо
...
class Var {
 public:
  ...
  static Var from_op(Tensor value, const char* name,
    ...
    apply_fp16_simulation(&value);
    return Var(std::move(value), std::make_shared<Node>(name, std::move(inputs),
                                                        std::move(backward)));
  }

  // То же для операции, результат которой — ВИД на буфер входа: reshape,
  // transpose, permute, slice, expand.
  //
  // Отличие одно: имитация пониженной разрядности не применяется. Вид делит
  // хранилище с владельцем, и округление записало бы результат в чужие данные
  // — в том числе в параметр, если вид сделан с него. Округлять параметры
  // имитация не должна: веса в половинной разрядности включаются отдельным
  // флагом, и замер сходимости показал, что обучение их не переносит.
  //
  // Терять на этом нечего: вид смотрит на значения, которые уже прошли
  // округление при своём появлении, а второе округление ничего не меняет —
  // это свойство проверено отдельным тестом.
  //
  // Различить вид и свежий результат может только сама операция. Попытка
  // определить это по тензору — «смотрит ли на буфер кто-то ещё» — уже была
  // сделана и оказалась неверной: операция вправе сохранить собственный выход
  // для обратного прохода, и тогда смотрящих тоже двое.
  static Var from_op_view(Tensor value, const char* name,
                          std::vector<NodePtr> inputs, BackwardFn backward) {
    return Var(std::move(value), std::make_shared<Node>(name, std::move(inputs),
                                                        std::move(backward)));
  }
