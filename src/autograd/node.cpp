...
void apply_fp16_simulation(Tensor* tensor) {
  ...
  // Только плотные И только собственные буферы.
  //
  // Плотности недостаточно, и это не придирка. Reshape, slice по первой оси,
  // select — всё это виды, которые ПЛОТНЫ, но делят хранилище с владельцем.
  // Запись в такой вид меняет данные владельца.
  //
  // Сейчас беды от этого нет: виды в модели делаются только с результатов
  // операций, а те уже округлены, и повторное округление ничего не меняет —
  // это проверено отдельным тестом. Но стоит однажды появиться виду на
  // ПАРАМЕТР, и имитация начнёт молча округлять сам параметр, чего она делать
  // не должна: веса в половинной разрядности — это отдельный флаг и отдельный
  // замер.
  //
  // owns_whole_storage требует единоличного владения и совпадения с буфером
  // целиком. Результат операции этому удовлетворяет: он только что выделен, и
  // Storage выделяет ровно запрошенное число байт.
  if (!tensor->is_contiguous() || !tensor->owns_whole_storage()) {
    return;
  }
