...
int main(int argc, char**) {
  ...
  check_exp();
  std::printf(
      "\nСтроки СВОИХ ядер обязаны совпадать между машинами, и слитные, и"
      " раздельные.\n"
      ...
      "\n"
      "Своих ядер не касается ровно одна строка — libm. Это чужая функция, а\n"
      "стандарт не определяет std::exp до последнего разряда (разбор в\n"
      "ops/fast_exp.h). Две машины с разными libc вправе разойтись в ней, и\n"
      "это не нарушение договора: он про то, что написано здесь, а не про\n"
      "чужую библиотеку. Строка печатается ради сравнения ВНУТРИ машины — со\n"
      "своим многочленом. На проверенных парах она пока совпадала, но\n"
      "совпадение это наблюдение, а не обязательство.\n");
  return 0;
}
