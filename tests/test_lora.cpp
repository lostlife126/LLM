...
LLM_TEST(Lora, MergeIsExactlyEquivalent) {
  ...
  //
  // Величина допуска замерена, а не взята с запасом на глаз. Настоящее
  // расхождение на этой модели — 7.5e-08 при масштабе логитов 0.46, то есть
  // на уровне машинной точности float; одинаковое под gcc и под clang.
  // Прежний допуск 2e-4 был больше него в две с половиной тысячи раз и
  // пропустил бы ошибку в слиянии, задевающую малую часть весов. Взято 1e-5 —
  // на два порядка больше замеренного, чтобы пережить другой порядок сложений
  // на другой машине, и на четыре порядка меньше того, что тест обязан
  // ловить: сам адаптер сдвигает логиты на 0.34.
  for (int64_t position = 0; position < 6; ++position) {
    for (int64_t token = 0; token < config.vocab_size; ++token) {
      LLM_EXPECT_NEAR(merged.value()(0, position, token),
                      with_adapter.value()(0, position, token), 1e-5);
    }
  }
