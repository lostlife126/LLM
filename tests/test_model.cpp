...
LLM_TEST(Model, BatchItemsAreIndependent) {
  ...
  //
  // Сравнение точное, и держится оно на том, как matmul_into разбирает
  // трёхмерный вход: он делает отдельное умножение на каждый элемент батча,
  // то есть m равно длине последовательности и от размера батча не зависит.
  // А от m зависит выбор пути в gemm, и пути делят глубину по-разному.
  //
  // Проверено: на модели покрупнее (d_model 256, FFN 1024) принудительная
  // смена пути двигает логиты на 8e-7, а переход с батча 1 на батч 2 не
  // меняет ни одного бита из трёх тысяч. Если однажды умножения по батчу
  // сольют в одно — а это на 4-10% быстрее, замерено, — точное равенство
  // здесь перестанет быть обязательным, и тест начнёт падать не потому, что
  // элементы батча увидели друг друга.
  const ModelConfig config = test_config();
