...
// Данные начинаются со смещения, кратного восьми.
//
// Наш читатель к этому безразличен: он берёт данные по смещению от конца
// заголовка и копирует их. Но файл, который мы называем safetensors, читают и
// другие, а тот, кто отображает его в память, обращается к массиву float
// прямо в отображении — и на невыровненном начале спотыкается.
//
// Проверяется здесь именно наше поведение, а не совпадение с чужой
// реализацией: её исходник не читался, и утверждать о ней что-либо было бы
// нечестно. Проверяется и то, что дополнение остаётся пробелами, а заголовок
// после них по-прежнему разбирается нами.
