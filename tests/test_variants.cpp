...
LLM_TEST(Variants, ZLossKeepsLogitsNearZero) {
  ...
  // Проверка на непустоту: без штрафа дрейф обязан быть заметным, иначе
  // сравнивать нечего и следующее условие выполнялось бы само собой.
  LLM_CHECK_MSG(drift[0] > 5.0f,
                "без штрафа логиты почти не уехали (" << drift[0]
                    << ") — сравнивать с чем-то нечего");

  // Порог по величине, а не только по направлению. Условие drift[1] < drift[0]
  // проверяет знак разности и больше ничего: ослабление штрафа в сто раз —
  // скажем, лишний множитель там, где коэффициент применяется, при неизменной
  // формуле градиента — направление сохраняет, и такая правка проходила мимо
  // всего набора тестов. Проверено порчей, а не рассуждением.
  //
  // Множитель 0.7 подобран замером: отношение здесь 0.449 (41.7 против 18.7),
  // так что запас до порога полуторакратный, а стократное ослабление, дающее
  // отношение около единицы, ловится.
  LLM_CHECK_MSG(drift[1] < drift[0] * 0.7f,
                "штраф ослабил дрейф логитов слишком мало: без него "
                    << drift[0] << ", с ним " << drift[1] << ", отношение "
                    << drift[1] / drift[0]);
}
