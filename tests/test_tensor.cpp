...
LLM_TEST(Tensor, BoundsChecksSurviveOverflow) {
  // Проверки границ написаны так, чтобы не переставать проверять на краю
  // диапазона. Оба случая недостижимы нынешними вызывающими, и проверка
  // закрепляет именно это — что защита остаётся защитой, а не превращается в
  // пропуск, если однажды в неё придёт вычисленная величина.

  // Срез: при записи start + count <= dim сумма двух int64_t переполняется и
  // становится отрицательной, то есть проверка пропускала бы срез, лежащий
  // целиком за пределами оси, и возвращала бы тензор с указателем в никуда.
  const llm::Tensor tensor = iota(llm::Shape({4, 3}));
  const std::int64_t huge = std::numeric_limits<std::int64_t>::max();
  LLM_EXPECT_THROWS(tensor.slice(0, huge, 1));
  LLM_EXPECT_THROWS(tensor.slice(0, 1, huge));
  LLM_EXPECT_THROWS(tensor.slice(0, huge, huge));
  // И обычный выход за границу по-прежнему отвергается.
  LLM_EXPECT_THROWS(tensor.slice(0, 3, 2));
  // А законный срез по-прежнему проходит: проверка не стала строже, чем была.
  LLM_CHECK_EQ(tensor.slice(0, 1, 3).dim(0), static_cast<std::int64_t>(3));
  LLM_CHECK_EQ(tensor.slice(0, 4, 0).dim(0), static_cast<std::int64_t>(0));

  // Выделение: число элементов проверено на переполнение в numel(), а байт
  // вчетверо больше. Форма ниже даёт numel(), помещающийся в int64_t, но
  // размер в байтах — уже нет: произведение на sizeof(float) заворачивается
  // по модулю 2^64, Storage получал бы обрезанное число и возвращал нулевой
  // указатель, а тензор оставался бы с полным numel().
  //
  // Число подобрано, а не взято «побольше», и первая попытка это показала.
  // Просто огромная форма отвергается и без проверки — но не ею, а проверкой
  // выравнивания внутри Storage, потому что завёрнутый размер всё равно
  // остаётся огромным. Тихим случай становится там, где произведение
  // заворачивается в МАЛЕНЬКОЕ число: 2^62 + 1 элементов дают ровно четыре
  // байта, Storage спокойно их выделяет, и наружу выходит тензор, который
  // обещает четыре квинтиллиона элементов и владеет одним.
  const std::int64_t wraps_to_four = (static_cast<std::int64_t>(1) << 62) + 1;
  LLM_EXPECT_THROWS(llm::Tensor::uninitialized(llm::Shape({wraps_to_four})));
}
